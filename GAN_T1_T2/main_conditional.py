...
from basic_GAN import Generator
from basic_GAN import Discriminator
...
for epoch in range(opt.n_epochs):
    ...
    for i, (T1, T2) in enumerate(data_loader_train):
        ...
        real = torch.cat((real_T1, real_T2), 1)
        fake = torch.cat((real_T1, fake_T2), 1)
        loss_D_real = criterionGAN(discriminator(real.detach()), True)
        loss_D_fake = criterionGAN(discriminator(fake.detach()), False)
        ...
        fake_2 = torch.cat((real_T1, fake_T2), 1)
        loss_G_GAN = criterionGAN(discriminator(fake_2), True)
    ...
    if epoch % 10 == 0:
        ...
        with torch.no_grad():
            ...
            for v, (v_T1, v_T2) in enumerate(data_loader_val):
                ...
                """ Discriminator Loss """
                v_fake = torch.cat((v_real_T1, v_fake_T2), 1)
                v_loss_D_fake = criterionGAN(discriminator(v_fake), False)
                v_real = torch.cat((v_real_T1, v_real_T2), 1)
                v_loss_D_real = criterionGAN(discriminator(v_real), True)
                v_loss_D = v_loss_D_real + v_loss_D_fake
                val_loss_D.append(v_loss_D)

                """ Generator Loss """
                v_fake_G = torch.cat((v_real_T1, v_fake_T2), 1)
                v_loss_G_GAN = criterionGAN(discriminator(v_fake_G), True)
...
with torch.no_grad():
    ...
    for t, (t_T1, t_T2) in enumerate(data_loader_test):
        ...
        """ Discriminator Loss """
        t_fake = torch.cat((t_real_T1, t_fake_T2), 1)
        t_loss_D_fake = criterionGAN(discriminator(t_fake), False)
        t_real = torch.cat((t_real_T1, t_real_T2), 1)
        t_loss_D_real = criterionGAN(discriminator(t_real), True)
        t_loss_D = t_loss_D_real + t_loss_D_fake
        test_loss_D.append(t_loss_D)

        """ Generator Loss """
        t_fake_G = torch.cat((t_real_T1, t_fake_T2), 1)
        t_loss_G_GAN = criterionGAN(discriminator(t_fake_G), True)
