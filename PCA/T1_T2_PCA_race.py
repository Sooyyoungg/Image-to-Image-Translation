...
def plot_site(data, method='PCA', target='race.ethnicity', norm=False, remove_zero_column=True, n_components=2):
    """
    :param data: DataFrame
    ...
    if norm == True:
        if not os.path.exists('/scratch/GANBERT/sooyoung/flattened_t1_t2_norm_race.npy'):
            flattened_images_T1_T2 = flatten(new_data, total_dimension, norm, remove_zero_column)
            np.save('/scratch/GANBERT/sooyoung/flattened_t1_t2_norm_race', flattened_images_T1_T2)
        else:
            flattened_images_T1_T2 = np.load('/scratch/GANBERT/sooyoung/flattened_t1_t2_norm_race.npy')
    else:
        if not os.path.exists('/scratch/GANBERT/sooyoung/flattened_t1_t2_wo_norm_race.npy'):
            flattened_images_T1_T2 = flatten(new_data, total_dimension, norm, remove_zero_column)
            np.save('/scratch/GANBERT/sooyoung/flattened_t1_t2_wo_norm_race', flattened_images_T1_T2)
        else:
            flattened_images_T1_T2 = np.load('/scratch/GANBERT/sooyoung/flattened_t1_t2_wo_norm_race.npy')
...
plot_site(output_final, method='PCA', target='race.ethnicity', norm=False, remove_zero_column=True)
gc.collect()