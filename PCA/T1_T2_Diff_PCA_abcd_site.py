...
def flatten(data, feature_dimension, norm=True, remove_zero_column=True):
    ...
    try:
        for sub in tqdm(range(data.shape[0])):
            ...
            Dti_image = nib.load(data.loc[sub, 'dti_directory'])
...
def plot_site(data, method='PCA', target='abcd_site', norm=False, remove_zero_column=True, n_components=2):
    """
    :param data: DataFrame
    ...
    if norm == True:
        if not os.path.exists('/scratch/GANBERT/sooyoung/flattened_t1_t2_norm.npy'):
            flattened_images_T1_T2 = flatten(new_data, total_dimension, norm, remove_zero_column)
            np.save('/scratch/GANBERT/sooyoung/flattened_t1_t2_norm', flattened_images_T1_T2)
        else:
            flattened_images_T1_T2 = np.load('/scratch/GANBERT/sooyoung/flattened_t1_t2_norm.npy',
                                             allow_pickle=True)
    else:
...
def plot_PCA(new_data, target, num_level, flattened_images, n_components):
    ...
    merged = pd.concat([result, target_info], axis=1)
    ...
    sns.scatterplot(data=merged, x='x', y='y', hue=target, s=20,
                    palette=sns.color_palette('muted', n_colors=num_level), )
...
### Main
output_final = pd.read_csv("/scratch/GANBERT/sooyoung/abcd_t1_t2_diff_info.csv")
plot_site(output_final, method='PCA', target='abcd_site', norm=False, remove_zero_column=True)